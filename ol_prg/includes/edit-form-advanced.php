<script>
  ...
  var switchEditors = {
    ...
    pre_wpautop: function (content) {
      ...
      // put back the line breaks in pre|script ффф
...
<div class="wrap">
    ...
    <form name="post"
          action="admin.php?page=<?php echo ORDER_LIST_PLUGIN_SLUG ?>&myaction=update"
          method="post" id="post">
        <?php
        ...
        <div id="poststuff"
            ...
            <div id="post-body">
                <div id="post-body-content">
                    ...
                    switch ($htmlelement) {
                        ...
                        case "checkbox";
                            ...
                            <input type="checkbox" name="<?php echo $columns[$i]["field"] ?>" size="30"
                                   tabindex="1" <?php echo $checked ?>/>
                        ...
                        case "textarea";
                            ...
                            <textarea name="<?php echo $columns[$i]["field"] ?>" size="30" tabindex="1"
                                      style="width:100%;height:<?php echo $columns[$i]["height"]; ?>;"><?php echo $textcontent ?></textarea>
                        ...
                        case "default":
                            ...
                            <div class="jcl_label"><?php echo $columns[$i]["header"] ?></div>
                            <input type="text"
                                   style="width:100%"<?php if ($columns[$i]["readonly"] == "readonly") echo 'readonly="readonly"'; ?>
                                   name="<?php echo $columns[$i]["field"] ?>"
                                   value="<?php echo esc_attr(htmlspecialchars($post->{$columns[$i]["field"]})); ?>"
                                   id="titlex" autocomplete="off"/>
				...
				<input class="button-primary" type="submit" name="finishbutton" value="Сохранить и выйти"/>
			</span>
    ...
    if (true) { ?>
        <div id="editor-toolbar">
            ...
            if (true) {
                ...
                <div class="zerosize"><input accesskey="e" type="button"
                                             onclick="switchEditors.go('<?php echo $id; ?>')"/></div>
                <?php if ('html' == $wp_default_editor) {
                    add_filter('the_editor_content', 'wp_htmledit_pre'); ?>
                    <a id="edButtonHTML" class="active hide-if-no-js"
                       onclick="switchEditors.go('<?php echo $id; ?>', 'html');"><?php _e('HTML'); ?></a>
                    <a id="edButtonPreview" class="hide-if-no-js"
                       onclick="switchEditors.go('<?php echo $id; ?>', 'tinymce');"><?php _e('Visual'); ?></a>
                <?php } else {
                    $class = " class='theEditor'";
                    add_filter('the_editor_content', 'wp_richedit_pre'); ?>
                    <a id="edButtonHTML" class="hide-if-no-js"
                       onclick="switchEditors.go('<?php echo $id; ?>', 'html');"><?php _e('HTML'); ?></a>
                    <a id="edButtonPreview" class="active hide-if-no-js"
                       onclick="switchEditors.go('<?php echo $id; ?>', 'tinymce');"><?php _e('Visual'); ?></a>
                <?php }
            }
