...
class ol_prg_table_class {
	...
	function getJoinSelect(){
	 ...
	 function get_select($id = 0, $select = "")
     {
		...
		if ($select == ""){
			$select = "SELECT ";
					...
					if ($this->getJoinSelect() != ""){
						$select .= $wpdb->prepare(" ".$this->getJoinSelect()." ",$id);
					}					
		...
		if ($this->getJoinStatement() != ""){
			$select .= $wpdb->prepare(" ".$this->getJoinStatement()." ",$id);
		}
		...
		if ($id != 0){
			...
			if ($this->getFilterStatement() != ""){
				$select .= $wpdb->prepare(" WHERE ".$this->getFilterStatement()." ",$id);
			}
			$select .= $wpdb->prepare($this->getOrderStatement(),$id);
			//echo $select;
            if (!stripos($select,'ORDER')) $select .= ' ORDER by date DESC';
	 ...
	 function do_add_defaults(){
		...
		$_post = $wpdb->get_row($wpdb->prepare($select),$id);
		return $_post;
	}
			...
			for ( $i = 1; $i <= sizeof($columns); $i++ ) {
					...
					switch ($htmlelement){
						...
						default:
							...
							$fieldcontent = nl2br(htmlentities($_POST[$columns[$i]["field"]],ENT_QUOTES,"UTF-8"));
							$sql .= $columns[$i]["field"]." = '".$fieldcontent."'";
	...
	function do_uninstall(){
		...
		// удаление таблицы после анинстала
		//$wpdb->query($sql);
