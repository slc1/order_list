...
function print_rows($this_table_obj)
{
    ...
    $posts = $this_table_obj->get_select_wrapper("rows");
        ...
        if (!isset($columns[$i]["display"]) || $columns[$i]["display"] != "none") {
            if (isset($columns[$i]["cell"])) {
                if ($columns[$i]["cell"] == "start") {
                    ?>
                    <th onclick="updateOrder(this, '<?php echo $sortfield; ?>')" style="<?php echo $textalign ?>" scope="col" <?php echo $width ?>>
                ...
                ?>
                <th onclick="updateOrder(this, '<?php echo $sortfield; ?>')" style="<?php echo $textalign ?>"
                    scope="col" <?php echo $width ?>><?php echo $header; ?>
                    <div class="<?php echo $sortclass ?>"></div>
                </th>
                <?php
            }
        }
...
function jcl_post_row($this_table_obj, $post)
{
    ...
    <tr id='post-<?php echo $post->ID; ?>'
        class='<?php echo trim($rowclass . ' author-' . $post_owner . ' status-' . $post->post_status); ?> iedit'
        valign="top">

        <th scope="row" class="check-column"><input type="checkbox" class="rowID" value="<?php echo $post->ID; ?>"/>
        </th>
        ...
        for ($i = 1; $i <= sizeof($columns); $i++) {
            ...
            //if (isset($columns[$i]["extra"]) && $columns[$i]["extra"] == "AUTO_INCREMENT" )	continue;


            if ((isset($columns[$i]["field"]) && $columns[$i]["field"] == "date") || (isset($columns[$i]["action"]) && $columns[$i]["action"] == "date")) {
                $attributes = 'class="post-title column-title"';

                $title = isset($columns[$i]["field"]) ? $post->{$columns[$i]["field"]} : $columns[$i]["field"] = "";
                ?>
                <td <?php echo $attributes ?>><strong><a class="row-title"
                                                         href="<?php //jcl_get_link($this_table_obj,$post->ID)
                                                         ?><?php echo ' ' . jcl_get_link($this_table_obj, $post->ID, "view") . '" title="' . esc_attr(__('Edit this post')); ?>"
                                                         title="<?php echo esc_attr(sprintf(__('Edit &#8220;%s&#8221;'), $title)); ?>"><?php echo $title ?></a></strong>
                    <?php
...
function jcl_get_link($this_table_obj, $id, $action)
{
    ...
    $action = "page=" . ORDER_LIST_PLUGIN_SLUG . "&myaction=" . $action . "&";
