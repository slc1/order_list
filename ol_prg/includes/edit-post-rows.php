...
function jcl_post_row($this_table_obj, $post) {
	...
	for ( $i = 1; $i <= sizeof($columns); $i++ ) {
		...
		?>
		<td <?php echo $attributes ?>><strong><a class="row-title" href="<?php //jcl_get_link($this_table_obj,$post->ID) ?><?php echo ' '. jcl_get_link($this_table_obj,$post->ID,"view") . '" title="' . esc_attr(__('Edit this post')); ?>" title="<?php echo esc_attr(sprintf(__('Edit &#8220;%s&#8221;'), $title)); ?>"><?php echo $title ?></a></strong>
		<?php
