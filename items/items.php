...
function rus2translit($string)
{
  ...
  $field_id1=(isset($_REQUEST["field_id1"]) ? $_REQUEST["field_id1"] : "");
    ...
    if ($is_rewrite_rules) {flush_rewrite_rules();  echo "<h2 style='color:red;'>Rules rewrited!!!</h2>"; }
...
if(! mysql_query( $sql) ) {  die('Error: ' . mysql_error()); }
 ...
 $table_name = $wpdb->prefix."ol_item_".mysql_insert_id();
...
if(!mysql_num_rows(mysql_query("SHOW TABLES LIKE '".$table_name."'"))==1) { 
      ...
      if(! mysql_query( $sql) ) {  die('Could not create table: ' . mysql_error()); }
	}
...
//------------------------------------------------------------------  
    if ($item_action=='add_item_field') include('add_item_field.php');
//------------------------------------------------------------------    
    if ($item_action=='add_item_field_to_base') {
        ...
        $mysql_res=mysql_query($sql);
         ...
         while (mysql_num_rows($mysql_res)>0) {
